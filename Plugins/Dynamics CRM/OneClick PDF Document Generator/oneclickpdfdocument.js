...
{
    var query = "$select=aspose_Parameter,aspose_Value&$filter=aspose_name eq 'OneClickPDF'";
        ...
        for (var i in results)
        {
            ...
            if (AsposeConfig.aspose_Parameter && AsposeConfig.aspose_Value)
            {
                if (AsposeConfig.aspose_Parameter == "URL")
                {
                    ...
                    var FullURL = URL + "OneClickPDFPage.aspx?id=" + id + "&orgname=" + orgname + "&typename=" + typename;
    ...
    if (results && results.length > 0) {
        for (var i in results) {
            ...
            if (AsposeConfig.aspose_Parameter && AsposeConfig.aspose_Value) {
                if (AsposeConfig.aspose_Parameter == "URL") {
                    var URL = AsposeConfig.aspose_Value;
                    var FullURL = URL + "OneClickPDFButton.aspx" + window.location.search;
