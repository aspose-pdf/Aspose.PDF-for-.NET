			...
			if (o.ShowViewerButton) {
				...
				if (ext !== null && viewerlink.length && VIEWABLE_EXTENSIONS.indexOf(ext) !== -1) {
					viewerlink.on('click', function (evt) {
						evt.preventDefault();
						evt.stopPropagation();
						openIframe(generateViewerLink(data), '/pdf/viewer', '/pdf/view');
					});
				}
				else {
					viewerlink.hide();
...
function requestMerger() {
	...
	let data = fileDrop.prepareFormData(minFiles, o.MaximumUploadFiles);
	if (data === null)
		return;

	let url = o.UIBasePath + 'Merger/Merger?inputType=' + 'pdf' + '&outputType=' + $('#saveAs').val();
	request(url, data);
}
function requestSplitter() {
	...
	let url = o.UIBasePath + 'Splitter/Splitter';
	request(url, data);
}
...
function requestXfaConversion() {
	let data = fileDrop.prepareFormData();
	if (data === null)
		return;

	let url = o.UIBasePath + 'XFA/XFA?inputType=' + getInputType('conversion');
	request(url, data);
}
...
function requestMetadata(data) {
	...
	if (response.length > 0) {
		let url = o.UIBasePath + 'api/AsposePdfMetadata/properties?folderName=' + response[2] + "&fileName=" + response[1];
		$.ajax({
			type: 'POST',
			url: url,			
