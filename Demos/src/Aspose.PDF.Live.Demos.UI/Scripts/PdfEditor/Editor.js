let apiBaseUrl = '/api/AsposePDFEditor/';
let documentId = '';
...
function DrawPic(imageId) {
    ...
    //}
    //else {
    //    image1 = `${apiBaseUrl}Image/${documentId}/${imageId}`.replace('.', '-');
    //    //image1 = 'Input/'.concat(imageId);
    //}

    image1 = `${apiBaseUrl}Image/${documentId}/${imageId}`.replace('.', '-');
    $("#imageView").css('background-image', 'url(' + image1 + "?Dummy=" + rand + ')');
    //$("#imageView").css('background-image', 'url(' + image1 + ')');
    return true;
...
function SendDelete() {
    $('#loadingModal').modal('show');
    var deleteData = JSON.stringify({ 'imageData': currentPage, 'imageName': Npages[currentPage - 1], 'documentId': documentId });
    return $.ajax({
        type: 'POST',
...
function Move() {
    ...
    if ($("#hdnMove").val() === "moving") {
        movedata = JSON.stringify({ 'moveFrom': currentPage, 'moveTo': moveTo, 'pageList': Npages, 'documentId': documentId });
        $('#loadingModal').modal('show');
        $.ajax({
            type: 'POST',
...
function InsertImages(data, imgLeft, imgTop) {
    ...
    let bRect = canvas.getBoundingClientRect();
    // image1 = 'Images/'.concat(data);
    let image1 = `${apiBaseUrl}image/${documentId}/${data.replace('.', '-')}?Dummy=${Math.random()}`;
...
function ExportByType(fileType, documentId) {
    ...
    //iframe.src = `${apiBaseUrl}export/${fileType}/${documentId}`;
    iframe.src = `${apiBaseUrl}download/${fileType}/${documentId}`;
...
function SaveToDisk() {
    window.open(`${apiBaseUrl}download/${documentId}`, '_blank');
}
...
function ExportFile(fileType) {
    ...
    $.ajax({
        type: 'POST',
        url: `${apiBaseUrl}/export-file/${fileType}/${documentId}`,
        data: { 'fileType': fileType, 'documentId': documentId },
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        success: function (data, textStatus, jqXHR) {
...
function saveSignature() {
    ...
    $.ajax({
        type: 'POST',
        url: `${apiBaseUrl}CreateSignature`,
